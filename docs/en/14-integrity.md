...
| `fk_orphan` | warning / info | a child FK value with no matching parent key: `warning` when the relation declares an enforcing `onDelete` (cascade/restrict/setNull — the enforcement was bypassed), `info` under `onDelete: noAction` (an accepted fact of the data); the severity follows `onDelete` alone, `onUpdate` does not affect it. A relation addressing a missing table/column or joining differently-typed columns (reachable only through hand-edits) is reported too; report-only |
