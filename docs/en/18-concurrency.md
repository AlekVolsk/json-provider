...
The cache layer serves reads only, on the "invalidate on write" model: DML through the provider clears the affected table's cache entry; mutations store a fresh snapshot only inside their EX section after the file is published. A lock-free reader also populates the cache on a miss (optimistically, without locks) — hence a narrow window exists in which a just-invalidated entry gets re-seeded with a snapshot read before the mutation; it lasts until the table's next mutation. For shared adapters (Redis, Memcached) full cross-process coherence is closed by the cache version tag (the "cache-coherence-version-tag" stage of the fix plan). When modifying the storage behind the provider's back (or from another process with the in-memory adapter), call `$db->invalidateCache($tableName)`.
