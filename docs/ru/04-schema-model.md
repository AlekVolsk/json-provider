...
- запись relation не объект, либо у неё нет строковых `from`/`foreignKey`/`to`/`references` (ловит опечатки вроде `form`), либо неизвестный `type` → `RELATION_ENTRY_INVALID`;
