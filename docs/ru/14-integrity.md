...
Если при создании инстанса передан PSR-3-логгер (`JsonDataProvider::getInstance($path, $cache, $logger)`), каждая находка прогона валидации логируется ровно один раз на уровне своего severity. Рантайм-деградации журналируются по той же шкале: тихий откат индексных чтений к full scan (byteSize-рассинхрон, легаси-формат) — `info`, бросок `INDEX_UNRELIABLE` — `error`, пропуск нечитаемых строк при restore — `warning`, деградация кэш-бэкенда — `warning` (логгер кэш-адаптера, см. [16-caching.md](16-caching.md)).
...
| `fk_orphan` | warning / info | FK-значение ребёнка без соответствующего родительского ключа: `warning` при объявленном enforce-действии (cascade/restrict/setNull — enforcement был обойдён), `info` при `noAction` (норма-факт данных); также репортится связь на несуществующую таблицу/колонку и типовой рассинхрон сторон (достижимо только ручной правкой схемы); report-only |
