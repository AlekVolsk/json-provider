...
Отображаются только промо-параметры конструктора **любой видимости** (`public`,
`protected`, `private`) — значения читаются через замыкание, привязанное к
области класса, поэтому non-nullable `private`-свойство с непустым значением не
даёт ложный `NULL_NOT_ALLOWED`. Унаследованные `private`-свойства
родительского класса не поддерживаются — маппируемое свойство объявляется на
самом DTO. Колонки, которых нет в DTO, подчиняются правилам массивного ядра
(пропуск nullable → `null`, пропуск non-nullable → ошибка на insert). Чтение —
без рефлексии на строку: карта (и её reader-замыкание) компилируется один раз
при регистрации.

Тип каждого маппируемого свойства обязан быть одиночным именованным: untyped,
`union`, `intersection` и `mixed` отклоняются на компиляции с точным
сообщением (`DTO_SCHEMA_MISMATCH`).
...
конвертация часового пояса — та же, что в массивном API: `datetime`/`datetimez`
хранятся в UTC и отдаются в локальном поясе, а `date`/`time`/`timez` — verbatim
(wall-clock), см. [Модель схемы → Типы даты/времени](04-schema-model.md#типы-датывремени-и-часовые-пояса).

## Имена — snake_case ↔ camelCase

По умолчанию свойство отображается на snake_case-форму своего имени, с корректным
разбиением аббревиатур: `createdAt` → `created_at`, `userID` → `user_id`,
`HTTPStatus` → `http_status`, `APIKey` → `api_key`. Для обычного случая атрибут не
нужен. `#[JsonProviderColumn("...")]` — штатный escape-hatch, когда имя колонки
стратегией не выводится (действительно нерегулярное):
...
Если выведенной колонки нет в схеме, сообщение об ошибке называет свойство и
подсказывает про `#[JsonProviderColumn]`.

## Один DTO на таблицу

К таблице привязывается ровно один DTO-класс. Повторная регистрация того же
класса — идемпотентный no-op; попытка привязать **другой** класс к той же таблице
→ `DTO_ALREADY_REGISTERED` (сначала снимите текущий через `unregister`).
