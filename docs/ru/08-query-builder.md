...
- chainable-методы (`where`, `condition`, `orderBy`, `limit`, `offset`, `distinct`, `setFilter`) накапливают состояние на `$this`;
...
Тот же доступ к данным без билдера и без состояния — [прямые методы провайдера](22-direct-provider-api.md).
...
Значения в `distinct` сравниваются строже, чем в `=`: `-0.0` и `0.0` — **два разных** значения, хотя `=`, unique-ограничения и индекс считают их одним. Знак нуля хранится на диске, и `distinct` показывает данные такими, какие они есть. Нужен один ноль — приведите значения при записи.
...
Логика условий **двузначная**, а не трёхзначная, как в SQL:

- сравнение с не-`null` значением (`=`, `>`, `<`, `BETWEEN`, `IN`, `LIKE`) строку с `null` в этом поле не находит — как в SQL;
- `where('f', '=', null)` находит строки с `null` — аналог `IS NULL`; с `not: true` — аналог `IS NOT NULL`;
- `not: true` — точное отрицание условия: в результат попадает **всё**, что условие не нашло, **включая строки с `null`**. В SQL `NOT (price > 10)` строки с `NULL` не вернёт (сравнение с `NULL` даёт UNKNOWN, и `NOT UNKNOWN` — тоже UNKNOWN), здесь — вернёт.

Если строки с `null` в отрицании не нужны, исключите их явно:

```php
$db->table('products')
    ->where('price', '>', 10, not: true)
    ->where('price', '=', null, not: true)   // только не-null
    ->selectAllByArray();
```
...
Порядковые операторы (`>`, `>=`, `<`, `<=`, `BETWEEN`) и `orderBy` для пар строк по умолчанию **байтовые** (`ComparisonModeEnum::Binary`): `'10' < '9'`, числовые строки не коэрцируются, порядок совпадает с байтовым порядком индексов — диапазон, сортировка и индексный путь дают один ответ. Числа сравниваются численно; `null` минимален; `=`/`IN` — строгое `===` всегда.

`$db->setComparisonMode(ComparisonModeEnum::Locale)` включает для пар строк коллатор ext-intl (естественно-языковой порядок). Режим влияет **только на порядок**: `=`/`IN`/`LIKE` остаются точными и индексируемыми, а сортировка/диапазоны по строковым колонкам перестают использовать индекс (байтовый порядок индекса несовместим с коллатором) и идут full scan'ом. Без ext-intl `Locale` тихо ведёт себя как `Binary`; зависимость объявлена в composer `suggest`.
