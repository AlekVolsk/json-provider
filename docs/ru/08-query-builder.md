...
LIKE **байтовый и регистрозависимый**. Единственный wildcard — неэкранированный `%` (любая последовательность байтов); `\%` — литеральный процент, `\\` — литеральный обратный слеш; `_` — **не** спецсимвол (матчит литеральный подчёрк). Регистронезависимого варианта (ILIKE) нет — приведение регистра остаётся на прикладном уровне. Для temporal-колонок шаблон сопоставляется с хранимой UTC-формой. Шаблон обязан быть строкой, колонка — строковой/temporal.
